...
void updateCourse(int clientSocket, struct userCred user) {
    struct courses temp;
    off_t recordoff = 0;
    char cname[100] = {0}; // Initialize with zeros
    int openFD = open("offered_courses_database.txt", O_RDWR, 0644); // Open for both reading and writing
    ...
    while (read(openFD, &temp, sizeof(temp)) > 0) {
        ...
        mylock.l_len = sizeof(struct courses);

        if (strcmp(user.loginId, temp.facultyID) == 0 && strcmp(cname, temp.courseCode) == 0) { // Compare the faculty IDs
            found = true;
            ...
            if (found) {
                ...
                char facultyID[100];
                if (sendPromptAndReceiveResponse(clientSocket, "Enter Faculty ID: ", facultyID, sizeof(facultyID)) == -1) {
                    close(clientSocket);
                    return;
                }
