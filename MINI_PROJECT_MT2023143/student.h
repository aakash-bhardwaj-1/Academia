...
bool authenticatestud(struct userCred user) {
    ...
    while (read(openFD, &temp, sizeof(temp)) > 0) {
        if (strcmp(user.loginId, temp.loginId) == 0 && strcmp(temp.activation,"1")==0) {
...
struct flock mylock;
     ...
     while (read(openFD_offered, &temp, sizeof(temp)) > 0) {
        ...
        // Lock the record
        if (fcntl(openFD_offered, F_SETLKW, &mylock) == -1) {
        ...
        mylock.l_type = F_UNLCK;
        recordoff += sizeof(struct courses);
        if (fcntl(openFD_offered, F_SETLK, &mylock) == -1) {
...
void dropCourse(int clientSocket){
  ...
  if(found==false){
        send(clientSocket, "You are not enrolled in this course. \n", strlen("You are not enrolled in this course. \n"), 0);
