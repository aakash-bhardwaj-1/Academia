...
bool admin_authenticate(int clientSocket){
struct userCred admin;
if (sendPromptAndReceiveResponse(clientSocket, "Enter Admin User ID: ", admin.loginId, sizeof(admin.loginId)) == -1) {
        close(clientSocket);
        return 0;
    }
    if (sendPromptAndReceiveResponse(clientSocket, "Enter Admin Password: ", admin.password, sizeof(admin.password)) == -1) {
        close(clientSocket);
        return 0;
    }
    
    if(strcmp(admin.loginId,"SSLAB")==0 && strcmp(admin.password,"iiitb")==0){
    return true;
    }
    else{
...
bool adminHandle(int clientSocket) {
    if (admin_authenticate(clientSocket)) {
    char readbuff[1024];
...
while (pread(openFD, &temp, sizeof(temp), recordoff) > 0) {
   ...
   send(clientSocket, buffer, strlen(buffer), 0);
        ...
        recordoff += sizeof(struct student);
    }
...
void activateStudent(int clientSocket){
    ...
    sprintf(buffer, "...................Previous Student Data..................\nName: %s\nLogin ID: %s\nPassword: %s\nActivation status(1 for active and 0 for blocked): %s\n.............................................\n",
                temp.name, temp.loginId, temp.password, temp.activation);
   send(clientSocket, buffer, strlen(buffer), 0);
 
 strcpy(temp.activation,"1");
...
void blockStudent(int clientSocket){
 ...
 strcpy(temp.activation,"0");
   ...
   send(clientSocket, buffer, strlen(buffer), 0);
        ...
        // Send the details to the client
        send(clientSocket, "Successfully blocked updated\n", strlen("Successfully blocked updated\n"), 0);
    } else {
        send(clientSocket, "Student not found\n", strlen("Student not found\n"), 0);
    }
